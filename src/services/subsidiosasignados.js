...
export async function getTotalSubsidio(idProyecto) {
  const url = `https://controlback.onrender.com/api/subsidiosAsignados/getTotalSubsidios/${idProyecto}`;
...
export async function getSubsidios(idProyecto) {
  const url = `https://controlback.onrender.com/api/subsidiosAsignados/getSubsidios/${idProyecto}`;
  const response = await fetch(url, {
    method: 'GET', // *GET, POST, PUT, DELETE, etc.
    mode: 'cors', // no-cors, *cors, same-origin
    cache: 'no-cache', // *default, no-cache, reload, force-cache, only-if-cached
